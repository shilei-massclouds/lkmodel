...
static mut ARGS: [[&str; 10]; 3] = [
    ["Linux_app1", "", "", "", "", "", "", "", "", ""],
    ["Linux_app2", "", "", "", "", "", "", "", "", ""],
    ["Linux_app3", "", "", "", "", "", "", "", "", ""],
];

static mut ENVS: [[&str; 10]; 3] = [
    ["", "", "", "", "", "", "", "", "", ""],
    ["", "", "", "", "", "", "", "", "", ""],
    ["", "", "", "", "", "", "", "", "", ""]
];
...
fn start_app( app_num:usize ){
    let apps_bin = PLASH_START as *const u8;
    let apps_bin_len = 32 * 1024 * 1024; 
    if let Some(app_pos) = find_marker(apps_bin, apps_bin_len, MARKERS[app_num]) {
        let app_start_address = PLASH_START as u64 + app_pos as u64 + MARKERS[app_num].len() as u64;
        print_bytes_at(app_start_address as usize, 5);
        load_app(app_start_address as usize, 0x2000000 , app_num);
    } else {
        println!("App1 marker not found.");
    }
}

fn load_app( offset : usize , len : usize , app_num:usize) {
    ...
    set_zero_page(app_tid);
}

fn set_zero_page(task_id:usize){
    let user_task = task::get_task(task_id).unwrap();
    match axalloc::global_allocator().alloc_pages(16, PAGE_SIZE ){
        Ok(memory_addr) => {
            let pa : usize = virt_to_phys(memory_addr.into()).into();
            let mm = user_task.mm();
            mm.lock().map_region(0 as usize, pa as usize, 16 * PAGE_SIZE, 0);
        }
        Err(err) => {
...
fn run_app( entry:usize  , app_num:usize) {
    let sp_top =  axalloc::global_allocator().alloc_pages(20, 4096 ).unwrap() + 18 * 4096;
    let mut new_sp = 0;
    unsafe {
        let args = ARGS[app_num];
        let env = ENVS[app_num]; // 假设的环境变量
        new_sp = elfloader::setup_stack(sp_top, 18 * 4096, &args, &env);
    }
    unsafe {
        core::arch::asm!("
            mv t0, {0}
